...
#  docker build --build-arg GPDB_VERSION=${GPDB_VERSION} --force-rm --squash -t ${DOCKER_OSS_TAG} -f DockerfileOpenSource .
  docker build --build-arg GPDB_VERSION="${GPDB_VERSION}" --build-arg GPDB_HOST="gpdbsne" --build-arg GPDB_BINARY="${GPDB_BINARY}" -t "${DOCKER_OSS_TAG}" -f DockerfileOpenSource .
